...
rendering, where it costs nothing to wait for the first visit. Credit
reconciliation is the exception: see "PWA & scheduled settlement" below for
why that path also gets a scheduled trigger.

`event_total` in league standings is not relied upon; net is derived from
`history`, which is unambiguous.

## PWA & scheduled settlement

Installable (`app/manifest.ts`), no offline caching — this app is live data,
not offline-first.
...
- Settlement itself is lazy on-visit: cheap and correct for rendering. But
  `reconcileCredit` staying broken for a week because nobody happened to open
  the app defeats the point of it, and Hobby cron's once-daily ±59 min
  precision doesn't help. So `.github/workflows/check-settled.yml` pings
  `/api/cron/check-settled` (guarded by `CRON_SECRET`) every ~10 min instead:
  free, no new account. Both paths funnel through the same `saveResult`
  `HSETNX`, so a real visitor and the scheduled check landing at once can't
  double-record.
- `CRON_SECRET` needs setting in Vercel's Production env vars and as a GitHub
...
- Two ways to resolve a pending entry, deliberately different: **Approve**
  ...
  `evicted:monzo:aliases` so every future credit from that name auto-applies
