...
export async function GET(request: Request) {
  ...
  // Deliberately not attempting to register the webhook here — see
  // lib/monzo/webhook.ts for why that has to be a separate, re-triable step.
  const response = NextResponse.redirect(new URL('/admin?monzo=connected', request.url));
