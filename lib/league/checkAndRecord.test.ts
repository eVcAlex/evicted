...
const { checkAndRecordSettled } = await import('./checkAndRecord');
...
async function run() {
  const result = await checkAndRecordSettled({ bootstrap, members, eligibleFrom: new Map() });
...
describe('checkAndRecordSettled', () => {
  ...
  it('defers the credit chase off the render path', async () => {
    ...
    await checkAndRecordSettled({ bootstrap, members, eligibleFrom: new Map() });
